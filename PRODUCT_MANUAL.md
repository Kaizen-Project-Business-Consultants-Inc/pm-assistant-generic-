...
- **Affected tasks** — tasks impacted by the change, each tagged with a severity badge (High / Medium / Low).
- **AI recommendations** — actionable mitigations or next steps generated by the analysis.

**API endpoint:** `POST /api/v1/intelligence/scenarios`

Request body fields: `projectId` (required), `scenario` (string description), `parameters` (optional object with `budgetChange`, `timelineChange`, `resourceChange`, `scopeChange`).

**Trial gate:** The What-If Scenarios endpoint remains fully hard-gated for trial users — no sample data is returned. Trial users who attempt to submit a scenario receive a standard upgrade prompt (HTTP 403 with `code: 'FEATURE_GATED'`).
