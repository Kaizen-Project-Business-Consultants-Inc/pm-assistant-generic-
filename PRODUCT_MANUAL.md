...
The Time tab includes intelligent anomaly detection, compliance monitoring, weekly review summaries, advanced analytics with data visualizations, and AI-powered features. Phase 1 features are rule-based (no AI tokens). Phase 2 features use the Claude API with graceful fallbacks when AI is disabled.

#### Time Anomaly Detection (Phase 1)
...
**AI Anomaly Explanations (Phase 2):** Managers/owners see an "Explain" button on each anomaly. Clicking it sends the anomaly + project context to Claude for root cause analysis and suggested actions. Falls back to static explanation lookup when AI is disabled.

#### Timesheet Compliance Engine (Phase 1)
...
#### Weekly Review Pack (Phase 1 + Phase 2)
...
#### Burndown Forecast (Phase 2)

Computes cumulative actual hours vs budget (sum of task `estimated_days * 8`), daily burn rate, and projected finish date. Displayed as an SVG line chart with ideal burn line (dashed gray), actual line (green/red), and projected extension. Summary cards show budget, actual, burn rate, and projected end date. Over-budget alert shown when applicable.

- **Sub-tab:** "Burndown" in the Time tab.
- **Endpoint:** `GET /time-entries/burndown/:projectId`

#### Trend Analysis (Phase 2)

Aggregates hours by ISO week over the last N weeks (default 12), computes week-over-week delta %, 4-week rolling average, and overall velocity trend (increasing/decreasing/stable). Displayed as an SVG bar + line chart with weekly bars, rolling average line, and delta badges.

- **Sub-tab:** "Trends" in the Time tab.
- **Endpoint:** `GET /time-entries/trends/:projectId?weeks=12`

#### Team Utilization Heatmap (Phase 2)

Shows a CSS grid with rows=users, columns=dates. Cell colors indicate utilization level: red (<25%), amber (25-74%), green (75-100%), dark red (>100%). Tooltips show hours and utilization percentage. Date range picker defaults to last 4 weeks. Per-user summary cards show average hours/day and utilization %.

- **Sub-tab:** "Heatmap" in the Time tab.
- **Endpoint:** `GET /time-entries/heatmap/:projectId?startDate=&endDate=`

#### Auto-Categorization (Phase 2)

Each time entry is automatically categorized based on task name/description keywords:

| Category | Keywords |
|---|---|
| Meeting | meeting, standup, review, sync, call, demo, retro |
| Admin | admin, planning, setup, onboard, document, report, status, email, timesheet |
...
#### Utilization Coaching (Phase 2, AI + Cron)
...
- Falls back to static coaching messages when AI is disabled.
...
| GET | `/time-entries/weekly-review/:projectId` | Weekly review pack with AI narrative (query: `weekStart`) |
| GET | `/time-entries/burndown/:projectId` | Burndown forecast with projection |
| GET | `/time-entries/trends/:projectId` | Trend analysis (query: `weeks`, default 12) |
| GET | `/time-entries/heatmap/:projectId` | Utilization heatmap (query: `startDate`, `endDate`, default 4 weeks) |
| GET | `/time-entries/suggest` | AI time suggestion (query: `projectId`, `date`) |
| POST | `/time-entries/anomaly-explain` | AI anomaly explanation (body: `{ anomaly, projectId }`) |
...
Lessons follow a four-stage lifecycle aligned with PMBOK 7 / PRINCE2 / ISO 21500 review practices:

| Status | Meaning |
|---|---|
| `draft` | Newly created or AI-extracted; awaiting review |
| `reviewed` | Reviewed by a team member; not yet formally approved |
| `approved` | Endorsed for organizational reuse; used in risk mitigation suggestions |
| `archived` | Superseded or no longer relevant; hidden from default views |

Only `approved` lessons are used by `suggestMitigations()` to prevent unvetted content from influencing risk guidance.

#### Contributor Tracking

Each lesson records `created_by` (user ID) and `source_type`:

- `manual` — entered by a team member
- `ai_extracted` — generated by AI closeout analysis
- `agent` — created by an autonomous agent
- `seeded` — pre-loaded reference content

#### Effectiveness Tracking

- **`applied_count`** — incremented each time a user applies the lesson as a mitigation suggestion. Indicates adoption.
- **`effectiveness_rating`** (0–100) — team-rated score reflecting whether the lesson produced measurable improvement.

#### Tags

Flexible JSON tags array for ad-hoc taxonomy (e.g., `["vendor", "scope", "estimation"]`). Searchable and filterable on the Lessons Learned page.

#### Persistent Patterns

Recurring patterns identified across the lesson database are stored in the `lesson_patterns` table (migration T026). They survive server restarts and grow as new lessons are added.

#### AI Extraction with RAID Context

When AI extraction runs at project closeout, the prompt is enriched with the project's RAID log (risks and issues), giving the model additional context for surfacing operationally grounded lessons.
...
The **What-If Scenario Modeling** panel lives on the Intelligence & Scenarios page (`/scenarios`). It is powered by `POST /api/v1/intelligence/scenarios` and provides a structured UI for running impact analyses without touching live project data.
...
- **4 impact cards**: Schedule Impact, Budget Impact, Resource Impact, and Risk Impact. Each card shows the before value, after value, and percentage change with color-coded directionality (red = negative, green = positive).
- **New risks identified** — list of risks that emerge from the scenario.
- **Affected tasks** — tasks impacted by the change, each tagged with a severity badge (High / Medium / Low). Tasks are scored using critical path analysis, float, overdue status, budget relevance, and timeline urgency — the top 8 most impacted tasks are shown.
- **AI recommendations** — actionable mitigations or next steps generated by the analysis.
- **Pin for comparison** — pin up to 3 scenario results, then view a side-by-side comparison table showing schedule, budget, workers, risk score, and confidence across all pinned scenarios.
- **Cross-link actions** — after running a scenario, buttons link to "Run Monte Carlo" (probability analysis on the schedule) and "Create Schedule Scenario" (clone the schedule with the T016 scenario infrastructure).
...
**Project-type awareness:** The analysis uses different risk coefficients for construction, IT, and consulting projects. Construction projects get higher schedule risk multipliers; IT projects get higher scope risk multipliers.

**Smart affected tasks:** Tasks are scored using critical path membership (30 pts), low float (10-20 pts), budget relevance, timeline urgency, scope sensitivity, and overdue status (25 pts). The top 8 tasks are returned with context-aware impact descriptions.
...
- **Resource auto-creation and linking** — before tasks are created, unique assignee names are matched (case-insensitively) against existing resources. New resources are created automatically with default settings (40 hrs/week, no role/department), and each imported task's `assignedTo` is stored as the matching **resource ID**, not the raw name, so the task modal, workload views and reports all see a real assignment. If resource lookup fails, the raw name is stored as a fallback. The import response includes a `resourcesCreated` count, shown in the UI as "X resources added."
- **Name fallback in pickers** — the Assigned To dropdown (task modal, Table view, Gantt) also resolves an `assignedTo` value by resource name when it is not an ID, so schedules imported before linking still show the correct resource.
...
**AI suggestions in Propose fixes — Phase 2 (September 2026).** `ScheduleFixProposerService.aiSuggestions()` makes **one** `completeWithJsonSchema` call per click (max 2,500 output tokens, temperature 0.2; ≤80 leaf tasks; skipped entirely when there is nothing to ask), asking only for what applies: phase groups (flat plan, 6+ loose tasks), **splits** (pre-filtered by `splitCandidates()` — non-milestone leaf tasks whose name contains *and / then / & / + / , / slash*), and **missing phases** (`findMissingPhases()`, shared with rule R31). The prompt carries the user's splitting rule and examples: split independent actions (hand-off, approval by someone else, separately trackable), keep one continuous activity; approvals and sign-offs become one-day milestones. Replies are validated by `buildSplitFixes()` / `buildPhaseFixes()`. New fix types: **`split_task`** — the task becomes a summary over 2–5 parts (dates shared by `planSplitDates()`, calendar days, largest remainder), parts chained FS, predecessors moved to the first part and successors to the last (lags kept), all reversible; **`add_task`** — one task after its anchor (same parent), FS from the anchor, and the "before" task linked to it; re-flow pushes later tasks. Plans without an AI allowance fall back silently to rule-based fixes; the panel says so. The score never uses AI.
