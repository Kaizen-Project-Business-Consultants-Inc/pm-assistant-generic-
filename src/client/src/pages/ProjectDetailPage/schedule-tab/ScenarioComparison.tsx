    ...
    <div className="mt-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <svg className="w-5 h-5 text-gray-500 dark:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onClose} className="text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 text-xs">Close</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div className="rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-3 text-center">
          <div className="text-xs font-medium text-gray-500 uppercase">Modified</div>
          <div className="mt-1 text-lg font-bold text-yellow-600">{data.summary.totalModified}</div>
        </div>
        <div className="rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-3 text-center">
          <div className="text-xs font-medium text-gray-500 uppercase">Added</div>
          <div className="mt-1 text-lg font-bold text-green-600">{data.summary.totalAdded}</div>
        </div>
        <div className="rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-3 text-center">
          <div className="text-xs font-medium text-gray-500 uppercase">Removed</div>
          <div className="mt-1 text-lg font-bold text-red-600">{data.summary.totalRemoved}</div>
        </div>
        <div className="rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-3 text-center">
          <div className="text-xs font-medium text-gray-500 uppercase">Duration Δ</div>
            ...
            <tbody>
              {data.diffs.map((d: any, i: number) => (
                <tr key={i} className="border-b border-gray-50 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className={`text-center px-2 py-1.5 font-medium ${(d.startDelta ?? 0) > 0 ? 'text-red-600' : (d.startDelta ?? 0) < 0 ? 'text-green-600' : 'text-gray-500'}`}>
                    {d.startDelta != null ? `${d.startDelta > 0 ? '+' : ''}${d.startDelta}d` : '—'}
                  </td>
                  <td className={`text-center px-2 py-1.5 font-medium ${(d.durationDelta ?? 0) > 0 ? 'text-red-600' : (d.durationDelta ?? 0) < 0 ? 'text-green-600' : 'text-gray-500'}`}>
