  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      >
        ...
        <p className="text-xs text-gray-500 mt-2">PDF, DOCX, DOC, TXT, CSV, MD (max 10MB) — documents are AI-analyzed for summaries, key points, and entity linking</p>
        {canConnect ? (
          <button
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {doc.tags && doc.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        ...
                        {doc.tags.length > 5 && <span className="text-xs text-gray-500">+{doc.tags.length - 5}</span>}
                      </div>
                    )}
                  </div>
    ...
    <div className="w-[45%] border rounded-lg p-4 bg-white dark:bg-gray-800 dark:border-gray-700 overflow-y-auto max-h-[70vh] space-y-4">
      ...
      <div>
        ...
        {editingDesc ? (
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            {doc.description || <span className="italic text-gray-500">No description</span>}
          </p>
        )}
      </div>
      ...
      <div>
        ...
        {editingFolder ? (
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            {doc.folder || <span className="italic text-gray-500">No folder</span>}
          </p>
        )}
      </div>
          ...
          <div className="space-y-1">
            {entityLinks.map((link: any) => (
              <div key={link.id} className="text-xs flex items-center gap-2 p-1.5 rounded bg-gray-50 dark:bg-gray-700/50">
                ...
                <span className="text-gray-500 truncate">{link.entityId}</span>
