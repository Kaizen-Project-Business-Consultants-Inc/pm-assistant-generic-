...
- **What-If Scenario Modeling panel** (`/scenarios`): project selector dropdown; 5 quick-preset buttons (Cut budget 20%, Add 30 days, Lose 2 team members, Increase scope 25%, Compress timeline 2 weeks); free-text scenario description; collapsible optional parameters (budget %, timeline days, workers ±, scope %); results show 4 impact cards (Schedule/Budget/Resource/Risk) with before/after values and % change, new risks identified, smart affected tasks scored by critical path/float/overdue/budget relevance (top 8) with High/Medium/Low severity badges, AI recommendations, confidence score, and AI-Enhanced badge when Claude contributes; pin up to 3 results for side-by-side comparison table; scenario history panel with mini impact summaries, pin-to-compare, and delete; cross-link buttons to Monte Carlo and Schedule Scenarios; project-type-aware risk coefficients (construction/IT/consulting); actual worker counts from task_assignments; persistent storage (T027 migration); trial users get sample data with amber upgrade banner; deterministic baseline with optional Claude AI enhancement
