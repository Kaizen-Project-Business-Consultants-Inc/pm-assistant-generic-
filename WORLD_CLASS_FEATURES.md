...
- **Per-task AC:** when tasks have `actualCost` recorded, the AC series sums per-task costs distributed proportionally to elapsed time, making CPI and cost metrics reflect real spend data; falls back to linear distribution of project-level `budgetSpent` when no per-task costs exist
...
- **Schedule Review (Phase 1)** — deterministic, DCMA-style schedule quality check with 28 rules (missing logic, milestones with duration, dates outside the project window, status/progress contradictions, placeholder dates, unowned tasks, duration-field mismatches, hard constraints, float, baselines, over-allocation, spreadsheet leftovers, hygiene). Produces a reproducible Schedule Health Score (0–100) with bands from *Tracking sheet* to *Fit for control*. Runs automatically after every import and on demand from the Review toolbar button; findings panel with per-finding "Show rows" grid filter, row indicators, eight-run trend, and stored history (`schedule_reviews`). No AI in this phase — rules find, the model will propose fixes in a later phase.
- **Import leak fixes (Schedule Review Phase 2)** — the import now preserves the structure the source file already holds instead of scoring it away: predecessors from a CSV column or MS Project links (parsed and resolved by UID, WBS code or task name in a second pass, cycles rejected, unresolved refs reported), milestone flags (milestone column or zero-duration rows), an automatic "Imported baseline" when the file carries baseline/actual dates, hours-vs-days detection from the working-day span, and removal of legend rows and spreadsheet cell references. The import summary reports links made, baseline captured, duration re-interpretation, skipped rows and warnings.
- **Structural fix proposals (Schedule Review Phase 3)** — the review drafts the repair, not just the diagnosis. **Propose fixes** offers concrete structural changes (add finish-to-start links, flag milestones, group tasks under a phase), each with a confidence and a plain-English reason; AI drafts them when available, deterministic rules otherwise (so every paid tier works). The PM ticks what to apply, a "Pre-review baseline" is captured, the fixes apply and the score re-computes, and a single **Undo** reverses the batch. Stored per schedule (`schedule_fix_proposals`), never executed autonomously, and dismissed proposals are remembered as negative examples.
...
- **Successor column (Succ)** — available in both Gantt left panel and Table View; shows which tasks depend on the current task (inverse of Predecessor); same row-number + type + lag format as Pred (e.g. `5FS`, `3SS+2d`); hover tooltip shows successor task names; read-only (derived from `task_dependencies`); hidden by default, toggle via column picker; visibility syncs between Gantt and Table views
...
- **Inline task insert (Gantt + Table)** — clicking the "+" row icon or right-click → Insert After opens an inline input row (green-tinted) below the target task instead of a modal; Enter creates, Tab creates + continues, Escape cancels; subtask-aware (inherits parent); Insert Before still uses modal
- **Row action icons (Gantt + Table)** — edit, insert-below (+), and delete icons on each row (hover to reveal); delete icon respects multi-select — deletes the full selection when multiple tasks are selected
...
- **Force change password** — admin-flaggable `must_change_password` gate that redirects users to a Change Password page on login and blocks all API calls (403) until the password is changed; auto-set for invite-created accounts
- Append-only chained audit ledger with API search, filter, and pagination
- Data encryption at rest and in transit
- Per-tier AI token budget enforcement (`AIBudgetService`) — tier-aware limits (Trial: 5K — ~10 AI chats to explore Mjuzi, Consultant Basic: 0 — no AI but includes resources/reports/workflows, Consultant Pro: 500K, SME: 1.5M, Enterprise: 5M), per-user admin overrides, purchasable token top-ups (500K/$10, FIFO consumption, no expiry), graceful degradation on exhaustion (HTTP 429 with reset date, non-AI features unaffected), `GET /api/v1/ai/budget` usage endpoint, automatic enforcement before every AI call, proactive 80% threshold warning notification (daily-deduped)
...
- **Narrative summary panel**: plain-English paragraph covering both cost and schedule performance — "The project is X% complete with Y% of budget spent. It is [schedule status] and [cost status]. [Forecast sentence including predicted outcome]." Covers % complete vs planned progress, budget performance (CPI), and projected project outcome. On Track / Needs Attention / At Risk badge and % Complete vs % Spent comparison bar (blue = complete, green/red = spent). AI prompt version 1.3.0. **Verified facts injection**: server pre-computes percentComplete (EV/BAC), percentPlanned (PV/BAC), scheduleStatus, budgetStatus, percentBudgetSpent (AC/BAC), schedule outlook, and forecast outcome; these are injected into the prompt as authoritative values that the AI writes prose around — guaranteeing narrative numbers and status labels match the actual dashboard metrics exactly.
- **Period-over-period deltas**: CPI and SPI cards display the week-over-week change inline (e.g., "+0.03" green / "−0.05" red) so performance trends are immediately visible without navigating to the trend chart
...
- **What-If Scenario Modeling panel** (`/scenarios`): project selector dropdown; 5 quick-preset buttons (Cut budget 20%, Add 30 days, Lose 2 team members, Increase scope 25%, Compress timeline 2 weeks); free-text scenario description; **interactive slider panel** with 4 always-visible range sliders (Budget %, Days +/-, Workers +/-, Scope %) with synced number inputs, per-slider reset, and Reset All button; **live impact preview** (2×2 grid): risk gauge (semi-circle arc with green/amber/red zones + baseline needle), timeline bar, budget bar, resource dots (blue retained, green gained, red faded lost); **sensitivity chart** (SVG line chart showing risk score across full parameter range, tab-switchable between Budget/Days/Workers/Scope, current value marked with dashed line + dot); client-side deterministic calculation using server-provided coefficients via `GET /scenarios/baseline/:projectId` — zero network latency on slider drag; "Enhance with AI" button for full Claude analysis with affected tasks, risks, and recommendations; pin up to 3 results for side-by-side comparison table; scenario history panel with mini impact summaries, pin-to-compare, and delete; cross-link buttons to Monte Carlo and Schedule Scenarios; project-type-aware risk coefficients (construction/IT/consulting); actual worker counts from task_assignments; persistent storage (T027 migration); trial users get sample data with amber upgrade banner
...
- **Agile EVM support**: projects with `methodology = 'agile'` compute PV/EV from sprint story points instead of task durations. `SCurveService.computeAgileScurve()` converts points to dollars via `budgetPerPoint = BAC / totalBacklogPoints`. PV steps by sprint end date (committed points), EV by completed points. All downstream metrics (CPI, SPI, EAC, warnings, AI) work identically. Dashboard shows an Agile EVM section with velocity trend chart, active sprint burndown, and sprint context stats (avg velocity, backlog completion %). Falls back to duration-based if no sprint data exists.
...
- Automatic sync every 15 min (configurable), adapter registry dispatches to correct provider API
...
- **AI Time Module** — Phase 1: anomaly detection (excessive hours, duplicates, weekend work, over-estimate, missing hours) with severity-coded alerts; daily compliance cron (16:00 weekdays) with escalation to managers after 3+ missed days; weekly review packs (Friday 17:00) with hours-by-user, anomaly counts, compliance %, over-budget tasks; automation events for time_entry CRUD and timesheet submission. Phase 2: burndown forecast (SVG chart with ideal/actual/projected lines, burn rate, projected finish date); trend analysis (weekly bar+line chart, rolling 4-week avg, velocity trend, delta badges); team utilization heatmap (CSS grid, color-coded cells, date range picker); auto-categorization (meeting/admin/productive pills + filter); AI smart time suggestions (pre-fill log form from patterns, Sparkles badge); AI anomaly explanations (root cause + suggested actions inline); AI weekly review narrative (2-3 sentence summary in blue callout); AI utilization coaching (Monday 09:00 cron, personalized tips for under/over-utilized members, Redis dedup, capped at 20/run). All AI features gracefully fall back when AI_ENABLED=false
...
- **Dashboard as default landing page** — authenticated users land on the Dashboard with the Morning Briefing widget, a 4-quadrant collapsible panel: **On Fire** (overdue tasks, critical/high risks — each a clickable row linking to the project's schedule or RAID tab), **Pending Approvals** (CRs, agent proposals — hidden for viewers), **Due Soon** (tasks due today/this week, upcoming milestones), and **RAID Watch** (blocked tasks, overdue action items, open issues). All items display the project name and link directly to the relevant project tab. Flashes amber on first daily visit (24h cooldown). Laid out 2×2 (September 2026): each box heading shows a count, On Fire is split into separate Overdue tasks and Risks sections (4 each), and every item is two lines — full name plus a coloured status tag, then project code · name · row · owner (owner for manager roles). Max 6 items per box with "+N more" overflow
- **Registration form hardening** — submit button disabled until Terms of Service is accepted and both password fields match; invite-flow registration shows contextual "join this organisation" text instead of generic copy
- **Benchmark:** Notion, Linear, Asana (all ship opinionated first-run flows)
...
- **Trial sample cross-project intelligence**: Portfolio Intelligence, Anomaly Detection, and What-If Scenarios endpoints return sample data with amber upgrade banner instead of a 403; Scenario Modeling page shows amber sample banner; no tokens consumed
...
- **Trial reminder emails**: dark-themed branded HTML templates (teal accent bar, logo, status badge, gradient CTA, 3 reassurance info points, responsive, dark-mode CSS, Outlook VML fallback); 3-day reminder, 1-day reminder, and expired notice; `buildTrialEmailHtml()` shared template builder in `EmailService.ts`
...
- Role-based permission matrix: all roles can create RAIDAD items (triage-gated for non-PM roles); admin=all operations including reverse; project_manager/scrum_master/pmo/ba=create + triage + cancel; reverse restricted to admin; **viewer** (assignment-based) — cannot create/cancel/reverse, but can update status, edit fields, post updates, and edit/delete own updates on items they are assigned as owner. Viewer assignment-based permissions also extend to: **tasks** (update status and comment on tasks assigned via resource linkage), **meeting action items** (update/complete/reopen/cancel items where `assigneeUserId` matches), **file attachments** (upload to assigned tasks and owned RAID items)
- **CSV/Excel Import**: bulk-load from `.csv`, `.xlsx`, `.xls` files; **multi-tab Excel support** — sheets named RAID, Risks, Issues, Actions, Decisions, Assumptions, Dependencies are auto-detected and imported by type; smart column mapper with exact alias, fuzzy, and AI-powered matching; value normalisation for type/severity/status/category; owner matching against project members; **smart date parsing** (DD-Mon-YYYY, MM/DD/YYYY, YYYY-MM-DD, and other common formats auto-converted); **title fallback** — if no Title column exists, the Description column (e.g., "Risk Description") is used as the title; max 200 rows per import; dedup by title; all records tagged `source: 'imported'`; aligned with DBJ spreadsheet column layout
...
- **AI-assisted authoring**: "Suggest with AI" buttons on Mitigation Plan, Trigger Condition, and Response Plan fields (**risk-only** — these fields are not shown on the issue form); uses RAG-based lesson retrieval + Claude to generate field-specific suggestions (early warning signs for triggers, contingency actions for response plans, preventive strategies for mitigations)
- **Mitigation Suggestions in Risk Form**: When creating or editing a risk, the `RiskFormModal` shows a collapsible **Suggested Mitigations** section (Shield icon) powered by `MitigationSuggestions` — historical mitigation strategies from past projects, ranked by relevance, with source project attribution
...
| Mjuzi Knowledge Base (RAG-powered product documentation search via `search_knowledge_base` tool; 5 doc files chunked by ### heading with breadcrumb context; max 800 words per chunk to stay within OpenAI's 8192-token embedding limit; embedded via OpenAI text-embedding-3-small; vector search via MariaDB VEC_DISTANCE_COSINE; `knowledge_base_chunks` table stores chunk text; admin reindex endpoint rebuilds after doc changes; Claude decides when to search docs vs use project data — migration 105, 8 files) | Done | AI/UX |
...
| Gantt Task Name Visibility Fix (default columns reduced from 12 to 6 essentials; Task Name now resizable at 250px default; left panel uses overflow-x:auto; resize handles show dot indicator) | Done | Fix |
| Successor Column — Gantt & Table (Succ column shows dependent tasks; row-number + type + lag format; hover tooltip with task names; read-only; hidden by default; visibility syncs between Gantt and Table views) | Done | Enhancement |
| EVM AI Narrative — Cost + Schedule (narrative summary now covers both cost and schedule performance: % complete vs planned, budget CPI, and projected outcome; prompt v1.2.0; predictedCPI/predictedSPI/correctiveActions made optional in Zod schema) | Done | Enhancement |
| EVM Per-Task AC + Verified Facts (S-curve AC now sums per-task actualCost distributed by elapsed time, falling back to linear budgetSpent; AI narrative uses server-pre-computed verified facts — percentComplete, percentPlanned, scheduleStatus, budgetStatus, percentBudgetSpent, outlook, forecast outcome — injected as authoritative prompt values; prompt v1.3.0) | Done | Enhancement |
...
| Dashboard as default landing page — Morning Briefing widget with 4 quadrants: On Fire (overdue tasks + critical risks), Pending Approvals (CRs + proposals, hidden for viewers), Due Soon (tasks + milestones), RAID Watch (blocked tasks, overdue action items, open issues). All items clickable with project name, linking to schedule/RAID tabs. 24h amber flash on first daily visit. Replaces former My Work page | Done | UX |
...
| Resend Invite (Resend button on Resources page for Pending/Expired invites; generates new token + 7-day expiry; rate-limited 20/min; existing-user detection redirects to login page with auto-accept after login via ?invite=TOKEN; POST /invites/:id/resend + POST /invites/accept (authenticated) endpoints) | Done | Enhancement |
| Viewer RAID Permissions — Assignment-Based (viewers who are the assigned owner of a RAID item can update status, edit fields, post updates, and edit/delete their own updates; viewers still cannot create, cancel, or reverse items; no seat consumption — external stakeholders manage their assigned risks/issues without a paid account) | Done | Enhancement |
| Viewer Assignment-Based Permissions Phase 2 (extends viewer bypass to 5 areas: task comment for assigned tasks via resource linkage, meeting action item update/complete/reopen/cancel for assigned items, file attachment upload to assigned tasks/RAID items, My Assignments dashboard endpoint, My Assignments widget with collapsible Tasks/RAID/Action Items sections) | Done | Enhancement |
| Viewer Read-Only Schedules + Sidebar Scoping (schedules fully read-only for viewers — all editing UI hidden; 10 sidebar items hidden from viewer/team_member roles: Resources, Meetings, CRs, Workflows, Intake, Analytics, EVM, Monte Carlo, Scenarios, Report Builder) | Done | Security |
| Password Reset UX Fixes (successful password reset auto-sets email_verified=true so users are not stranded by email-verification gate; ResetPassword page shows "Request a new reset link" on expired/invalid token instead of a dead-end error) | Done | Fix |
...
| Import Column Mapper — MPP Labels & Skip Fix (dropdown labels match Microsoft Project conventions: Task Name, Start, Finish, Resource Names, % Complete, Duration, Notes; MPP aliases added server+client; skip now works reliably — all unmapped columns default to `_skip`; server returns null for `_skip` fields) | Done | Fix |
| Mjuzi Live UI Sync (React Query cache invalidation for tasks/schedule/gantt/projects after Mjuzi mutating actions; task highlight on creation with yellow flash + scroll-to-view; delete cleanup clears activeTaskId; sortOrder included in create_task action result for accurate position reporting) | Done | Enhancement |
| Automation Engine (event-driven rules engine: 21 trigger event types across 6 entities (task/project/risk/sprint/change_request/proposal); 14 condition operators with nested AND/OR groups; 11 action types (create_task, notify, send_email, add_risk, change_status, update_field, add_comment, escalate, call_webhook, log_audit, auto_assign); safety guards: 50 runs/day cap, cooldown seconds, recursion depth limit of 3; enable/disable toggle; dry-run/test capability; execution history log; 10 API endpoints under /api/v1/projects/:projectId/automations/) | Done | Enhancement |
...
| Schedule Quick Filter Pills & At-Risk Visual Highlighting (7 one-click filter pills with live badge counts: All/Due This Week/Due 2 Weeks/Late/At Risk/My Tasks/Unassigned; red left border/bar border/card border for late tasks, orange for at-risk; risk assessment compares expected progress from elapsed time vs actual progress_percentage; configurable thresholds via gear icon (At Risk % gap default 20, Critical % gap default 50); filter persists to localStorage and URL param ?qf=<filter>) | Done | Enhancement |
| Doc Intelligence Tab Rename (renamed "Documents" tab to "Doc Intelligence" in project detail overflow tabs to disambiguate from the Files/Attachments tab; updated tab label in methodology.ts, search placeholder, upload helper text, and empty state messaging) | Done | UX |
| React Query Cache Key Standardization (comprehensive audit and fix of 18 mismatched queryKey strings across 16 files; aligned fetch keys with invalidation keys so mutations correctly refresh all related components; 12 key families standardized: evm, meetingHistory, project-risks-stats, audit-trail, project-members, resource-forecast, workload, histogram, dashboard-predictions, analytics-summary, projects, project-risks) | Done | Fix |
| UI Typography Consistency Audit (full-app font audit and standardization across 28 files; page titles unified to text-xl; tab section headings to text-base; card headings to text-sm font-semibold; modal titles to text-lg font-semibold; widget titles standardized to text-sm font-semibold text-gray-900 dark:text-gray-100; inline form headings aligned from font-bold to font-semibold; semantic heading elements — span→h3 for widget titles, p→h4 for column headers; table header tracking standardized to tracking-wider) | Done | UX |
| WCAG 2.2 AA Full Audit (20-finding W-series audit — all addressed; W1 high-contrast dark mode compound selectors + cascade fix; W2 focus-visible rings on 4 inline editors; W3 table semantics scope/aria-sort/role="grid"/captions on 8 tables; W4 keyboard shortcuts disable toggle in Accessibility settings; W5 modal inert+aria-hidden on #main-content + createPortal + document Escape; W6 aria-label on ScheduleTab icon buttons; W7 aria-labelledby on 3 accessibility switches; W8 announce() live region utility + 23 call sites; W9 focus-ring suppression narrowed to ring-2/ring-4 only; W10 Settings tablist with aria-selected/aria-controls/roving tabindex; W11 LoginPage autoComplete + aria-invalid + focus on state change + h1 + main wrapper; W12 font-size as % multiplier to 200%; W13 SW toast role="status" + aria-live + aria-label; W14 keyboard Move Up/Down for dashboard widget reorder; W15 AI narration aria-live="polite"; W16 simplificationLevel confirmed no consumer; W17 FOCUSABLE selector expanded + hidden/disabled filter; W18 focus restoration on unmount; W19 no-op rule deleted; W20 sr-only clip-path:inset(50%) — 45+ files) | Done | A11y |
| WCAG 2.2 AA Final Pass (remaining 8 findings from re-audit — W1 @media prefers-contrast:more OS-level rules; W2 focus:ring-2 on all 10 Gantt cell editors; W3 role=grid/row/columnheader on Gantt task table; W6 role=toolbar+aria-label on ScheduleTab; W11a role=alert on 5 toast notifications; W12 Alt+ArrowUp/Down keyboard reorder for Gantt rows; W14 aria-describedby on Register/ForgotPassword/ResetPassword forms; W18 sr-only h2 for tab heading hierarchy; W20 notification dismiss button 24px+ target — 12 files) | Done | A11y |
| UI Audit Phase 1+2 Design System (ai.* tokens retuned from teal to violet; status.* token group added with 5 semantic colors; tier badge colors neutralized to gray; shared severityColor utility deduplicated from 7 inline copies to 1 shared import — 5 files) | Done | Visual System |
| Mjuzi PM Methodology Expertise (explicit PM methodology knowledge in Mjuzi system prompt; covers PMBOK 10 knowledge areas + 5 process groups, PRINCE2, Agile: Scrum/Kanban/SAFe/XP, Lean, Critical Chain, hybrid approaches; core techniques: EVM, risk registers, WBS decomposition, stakeholder analysis, CPM, resource leveling; responds at beginner-to-advanced depth based on question framing) | Done | AI/UX |
| Pinned Project Links — Key Links card (project Overview tab; CRUD with inline add/edit form; 12 icon presets with color-coded badges: SharePoint/Jira/Confluence/Figma/GitHub/Google Drive/Slack/Teams/Notion/Miro/Trello/Generic; drag reorder via PUT /links/reorder; viewer read-only, editor+ manage; renders after Project Brief card; migration T042_project_links.sql; 5 API endpoints) | Done | Enhancement |
| Project Allocation Column — Resource Management Team tab (each resource row shows assigned projects as compact chips with project name + hours; summary shows total allocated vs weekly capacity with green/red color-coding; data joined from task_assignments + resource_assignments) | Done | Enhancement |
| MPP-Style Resource Usage — Project Detail Team tab (expandable resource rows showing per-task breakdown: hours, allocation %, role, dates, status badge; collapsed header shows total hours + utilization; mirrors MS Project Resource Usage view) | Done | Enhancement |
| Auto-Create User on Invite (admins inviting unregistered users trigger automatic account creation with a cryptographically secure temporary password; password included in invite email; forced Change Password screen on first login before app access; existing accounts follow standard acceptance flow) | Done | Enhancement |
| Invite Role Restrictions by Tier (Consultant tiers limited to viewer-only invites — UI hides role dropdown and backend rejects non-viewer with 403; SME/Enterprise non-viewer invites auto-add a paid seat via Stripe if needed; role changes on existing members follow same rules; amber UI note for seat consumption; shared `isConsultantTier()` utility) | Done | Enhancement |
| Automation Marketplace Templates (18 pre-built automation recipes seeded into marketplace: 3 Deadline Management, 2 Status Workflow, 2 Risk & Quality, 3 Team Communication, 2 Resource Management, 4 AI-Powered incl. sprint retro/risk mitigation/task enrichment/lesson extraction, 2 Compliance & Audit; migration 112; users browse and import into projects on demand) | Done | Enhancement |
| Duplicate Index Cleanup (21 duplicate indexes removed across control plane + tenant DBs via migrations 113 + T043; reduces write I/O overhead; identified via production slow query audit) | Done | Performance |
| Context Engineering (4 pillars: hierarchical AI context config org→project→user with locked keys; dreaming/batch memory refinement from conversation analysis with auto-apply threshold; versioned permissioned memory with SHA-256 hash-based optimistic locking, audit trail, and rollback; progressive skill disclosure with front-matter summaries in agent context + on-demand detailed procedures; Settings > AI Context tab with Preferences/Memory Browser/Dreaming/Context Preview sub-tabs; nightly dreaming cron at 02:30; 6 new tables + ALTER agent_memory + 1 tenant table — migrations 108 + T041, 4 services, 4 routes, 3 frontend components, 49 tests) | Done | AI/Context |
