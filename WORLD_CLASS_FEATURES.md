...
| Automation Engine (event-driven rules engine: 21 trigger event types across 6 entities (task/project/risk/sprint/change_request/proposal); 14 condition operators with nested AND/OR groups; 10 action types (create_task, notify, send_email, add_risk, change_status, update_field, add_comment, escalate, call_webhook, log_audit); safety guards: 50 runs/day cap, cooldown seconds, recursion depth limit of 3; enable/disable toggle; dry-run/test capability; execution history log; 10 API endpoints under /api/v1/projects/:projectId/automations/) | Done | Enhancement |
