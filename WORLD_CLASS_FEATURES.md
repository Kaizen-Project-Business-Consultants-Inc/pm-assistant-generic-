...
- **My Work smart empty states** — `/my-work` shows a tailored welcome screen for users with no projects yet (Create a Project / Use a Template CTAs), and an "all caught up" state for users who have projects but no current assignments; task rows use proper `<Link>` elements for full keyboard and screen-reader accessibility
- **My Work date bucketing** — tasks bucketed by local time (not UTC), so "Due Today" and "Overdue" reflect the user's clock; in-progress tasks with no due date correctly appear in the In Progress bucket rather than being dropped
