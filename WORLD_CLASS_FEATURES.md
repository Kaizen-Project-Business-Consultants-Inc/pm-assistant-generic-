...
- **What-If Scenario Modeling panel** (`/scenarios`): project selector dropdown; 5 quick-preset buttons (Cut budget 20%, Add 30 days, Lose 2 team members, Increase scope 25%, Compress timeline 2 weeks); free-text scenario description; collapsible optional parameters (budget %, timeline days, workers ±, scope %); results show 4 impact cards (Schedule/Budget/Resource/Risk) with before/after values and % change, new risks identified, affected tasks with High/Medium/Low severity badges, AI recommendations, confidence score, and AI-Enhanced badge when Claude contributes; deterministic baseline with optional Claude AI enhancement; fully gated on trial (no sample data)
