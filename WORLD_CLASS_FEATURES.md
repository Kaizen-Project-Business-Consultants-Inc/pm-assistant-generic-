...
- Role-based permission matrix: all roles can create RAIDAD items (triage-gated for non-PM roles); admin=all operations including reverse; project_manager/scrum_master/pmo/ba=create + triage + cancel; reverse restricted to admin; **viewer** (assignment-based) — cannot create/cancel/reverse, but can update status, edit fields, post updates, and edit/delete own updates on items they are assigned as owner
