...
- **My Work Personal Command Center** — `/my-work` redesigned as a 5-zone unified hub: (1) Attention Bar (red/amber banner for overdue/blocked/critical risks, auto-hides when clear), (2) Needs Your Decision (timesheet/resource request inline approve/reject, change requests, agent proposals), (3) My Commitments (unified queue merging tasks + meeting action items + RAID actions with tab filtering, project filter, and time buckets), (4) Looking Ahead (5 business-day calendar with milestone markers), (5) Recently Completed (last 7 days, starts collapsed). Backend runs 9 parallel queries via Promise.all. Inline Mark Done with optimistic updates routes to correct API per item type.
- **My Work smart empty states** — "You're all clear" greeting when nothing needs attention; welcome screen for new users with Create a Project / Use a Template CTAs
- **My Work date bucketing** — items bucketed by local time (not UTC), so "Due Today" and "Overdue" reflect the user's clock; in-progress items with no due date correctly appear in the In Progress bucket
...
| My Work page (/my-work) — cross-project personal task aggregation; default post-login landing page; triple matching: resource ID + free-text name LIKE + task_assignments junction; auto-links resources to users by email on create/update/member-add with email fallback on load; 6 priority buckets (Overdue/Due Today/Due This Week/In Progress/Upcoming/Recently Completed); collapsible sections with count badges; task rows show priority dot, task name, linked project name, due date, status chip; click row navigates to project schedule with task selected; smart empty states (welcome screen with Create a Project / Use a Template CTAs for new users; "all caught up" for users with projects but no assignments); local-time date bucketing (not UTC); in-progress tasks with no due date correctly bucketed; proper &lt;Link&gt; elements in task rows for keyboard and screen-reader accessibility; first item in sidebar Work section | Done | UX |
