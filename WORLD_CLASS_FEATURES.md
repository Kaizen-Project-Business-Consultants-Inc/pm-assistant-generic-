...
- **Dashboard as default landing page** — authenticated users land on the Dashboard with the Morning Briefing widget, a 4-quadrant collapsible panel: **On Fire** (overdue tasks, critical/high risks — each a clickable row linking to the project's schedule or RAID tab), **Pending Approvals** (CRs, agent proposals — hidden for viewers), **Due Soon** (tasks due today/this week, upcoming milestones), and **RAID Watch** (blocked tasks, overdue action items, open issues). All items display the project name and link directly to the relevant project tab. Flashes amber on first daily visit (24h cooldown). Max 5 items per quadrant with "+N more" overflow
