...
| Mjuzi Knowledge Base (RAG-powered product documentation search via `search_knowledge_base` tool; 5 doc files chunked by ### heading with breadcrumb context; embedded via OpenAI text-embedding-3-small; vector search via MariaDB VEC_DISTANCE_COSINE; `knowledge_base_chunks` table stores chunk text; admin reindex endpoint rebuilds after doc changes; Claude decides when to search docs vs use project data — migration 105, 8 files) | Done | AI/UX |
...
| Resend Invite (Resend button on Resources page for Pending/Expired invites; generates new token + 7-day expiry; rate-limited 20/min; existing-user detection redirects to login with auto-accept; POST /invites/:id/resend + POST /invites/accept endpoints) | Done | Enhancement |
