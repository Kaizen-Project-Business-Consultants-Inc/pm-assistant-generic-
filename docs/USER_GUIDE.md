...
- **Row action icons**: Each row shows three action icons on hover — **Edit** (pencil, opens the task editor), **Insert Below** (+ icon, inserts a blank inline row below where you type a task name and press Enter to create it in place), and **Delete** (trash, opens a confirmation modal before deleting; if multiple tasks are selected, the trash icon deletes the entire selection). Icons fade in on row hover.
- **Inline task insert**: Clicking the **+** button or using right-click → **Insert After** opens an inline input row (green-tinted) directly below the target task — no modal. Type a task name and press **Enter** to create the task at the correct position. Press **Tab** to create and immediately start another (continuous entry, like MS Project). Press **Escape** or blur with empty input to cancel. The new task inherits the parent of the target row (subtask-aware). **Insert Before** still opens the full task form modal.
...
**Successor column (Succ)**: Enable via the Columns picker. Shows which tasks depend on the current task (the reverse of Predecessor). Uses the same compact row-number format with type and lag (e.g. `5FS`, `3SS+2d`). Hover to see a tooltip with successor task names. Read-only — edit the dependent task's Predecessor field to change relationships.
...
**Right-Click Context Menu** -- Right-click any task row to open a context menu with: Insert Before, Insert After, Edit Task, Indent, Outdent, and Delete. **Insert After** uses inline insert (a blank row appears below for quick name entry). When multiple tasks are selected, Delete shows the total count and removes all selected tasks plus the right-clicked task.

**Inline Task Insert** -- Each Table row shows a **+** icon on hover (next to Edit and Delete). Clicking it inserts a blank input row below. Type a name, press **Enter** to create, **Tab** for continuous entry, **Escape** to cancel. Same behavior as the Gantt view.
...
### Lesson Status and the Review Workflow

Every lesson has a status that tracks its maturity. Managers can advance lessons through the workflow using the buttons on each lesson card.

| Status | Color | Meaning |
|---|---|---|
| Draft | Grey | Newly created or AI-extracted; not yet reviewed |
| Reviewed | Blue | Reviewed by a team member; awaiting formal sign-off |
...
Each lesson card shows an icon indicating how the lesson was created:

- **Sparkle icon** -- AI-extracted (generated by AI analysis at project closeout)
- **Bot icon** -- Created by an autonomous agent
- **Database icon** -- Pre-loaded reference content (seeded)
- **User icon** -- Entered manually by a team member

### Effectiveness Tracking

After applying a lesson, you can rate how effective it was:

- **Applied count** -- Automatically incremented when you apply a lesson as a mitigation suggestion. Reflects adoption across your organization.
- **Effectiveness rating** -- A score from 0 to 100 you can set on a lesson card to record whether it produced measurable improvement. Use this to surface the lessons that actually make a difference.
...
The AI analyzes your lessons learned database and identifies recurring patterns. Patterns are stored persistently so they are available immediately on page load without re-running analysis.
...
**UI Audit Sprint 1 (July 2026):** Nine features that previously had no sidebar navigation entry (Monte Carlo, Scenario Modeling, Workflows, Report Builder, Intake Forms, Integrations, Agent Proposals) are now accessible from the sidebar under their respective sections (Manage and Insights). The breadcrumb system has a complete label map for all routes — short segments like "goals" and "query" are no longer incorrectly uppercased. KPI tiles on the dashboard are now proper links (keyboard-navigable with focus ring). The dashboard defaults to 4 widgets (KPI tiles, Projects table, Morning Briefing, AI Suggestions) instead of 13 — additional widgets are available via the Customize dropdown. The "Last refreshed" timestamp now shows actual data age ("updated 4 min ago") instead of the render time. The PWA theme color matches the app's teal brand. A global keyboard focus ring is now visible across the entire application when tabbing. Dark mode links show an underline on hover for discoverability.
...
**Schedule Review:** As soon as an import finishes, Kovarti checks the schedule the way a PMO would and shows a **Schedule Health Score** from 0 to 100 with the top findings, for example "None of the 28 tasks are linked" or "Gate 1 spans 10 days. A milestone is a decision on a single day." Click **Review and fix** to see the full list. You can also run it any time from the **Review** button in the schedule toolbar (next to Columns).

In the panel, findings are grouped by severity. Each one says what is wrong in plain English and has a **Show rows** button that filters the grid to exactly the tasks involved; use **Clear** in the orange bar above the grid to see everything again. Tasks with a Critical or High finding show a small orange dot beside their row number. The review also **re-runs by itself about 20 seconds after you stop editing** the schedule (adding, changing, deleting or linking tasks), so the panel and the Schedule Health card stay current — it's a set of rules, not AI, so it uses no AI tokens. **Re-run review** refreshes the score straight away, and the small bar chart shows how the score has moved over the last eight runs.

**Checks for your kind of project** (rules v1.2, September 2026). Set the **Project Type** in **Edit** on the project page — *IT / Software*, *Web Design*, *Web Application* or *App Development* — and the review also checks:

- **Task length** — work longer than **15 working days** (IT) or **10 working days** (web and app) as a single task is flagged; other project types keep the general 44-day limit. Ongoing overhead such as status reporting, project management, meetings or hypercare is never flagged for length.
- **Standard phases** — whether the plan has the phases that kind of project normally needs. IT run as Waterfall: Requirements, Design, Build, Testing, Deployment. IT (or web/app) run as **Agile**: Backlog/discovery, Sprints (sprints set up on the project count), Testing, Release. Web Design: Discovery/brief, Wireframes/UX, Visual design, Content, Review and approval, Launch/handover. Web Application: Discovery/requirements, UX/UI design, Front end, Back end/API, Testing, Deployment. App Development: Requirements, UX/UI design, Development, Testing, Store release. Matching is on task names, and milestones don't count as the work itself.
- **Key milestones** — e.g. requirements/design sign-off, UAT sign-off and go-live for IT; design approved and site launch for web design; design approved, beta/TestFlight and store release for apps.
- **Summary tasks** — a summary task with its own predecessor or successor (the link belongs on the task inside it), and a summary whose dates don't cover the tasks under it.
...
14. Use the **What-If Scenario Simulator** (collapsible section) to explore budget and performance scenarios. Drag the **Target CPI** slider (0.5–1.5) and **Budget Adjustment** slider (±30–50% of BAC) to see EAC, ETC, VAC, and TCPI recalculated in real-time. Current and simulated values are shown side by side with color-coded deltas. Click **Reset to actuals** to restore original values. No data is saved — this is purely exploratory.
15. The **Variance Breakdown (Pareto)** section shows which tasks contribute most to cost variance. A horizontal bar chart displays the top 10 tasks by absolute cost variance — green bars for tasks under budget, red for over budget, centered on zero. Below the chart, a compact table lists each task's name, budget, actual cost, CV (cost variance), SV (schedule variance), and progress %.
16. The **Earned Schedule (ES) Metrics** panel displays time-based schedule performance computed from the S-curve data: ES (earned schedule in weeks), AT (actual time elapsed), SV(t) = ES − AT (schedule variance in time units), and SPI(t) = ES / AT (schedule performance index in time). A plain-English interpretation explains what the numbers mean. Unlike dollar-based SV which converges to zero near project end, SV(t) remains meaningful throughout the project lifecycle.
17. The **TCPI Dual Target Analysis** compares the TCPI needed to finish on the original budget (BAC) vs the current forecast (EAC), displayed side by side. Values are color-coded: green (<1.05 — achievable), amber (1.05–1.2 — challenging), red (>1.2 — very difficult). When the BAC-based TCPI is unrealistic but the EAC-based TCPI is achievable, an amber callout suggests considering a rebaseline.
...
- **+ Issue** — Opens the Issue form. Issues have their own fields distinct from risks: title, description, severity, category, owner, root cause ("Why did this happen?"), impact assessment, workaround ("Temporary fix"), resolution plan ("Permanent fix"), and target resolution date. Trigger Condition, Mitigation Plan, and Response Plan are **risk-only** fields and do not appear on the issue form. Probability is not shown since the issue has already occurred. Assigned an `I-NNN` ID.
...
4. **Import** — click the Import button. The server normalises values (e.g., "High" → high, "R" → risk, "In Progress" → in_progress) and matches Owner names to project members. If an owner name does not match a project member it is saved as a free-text fallback. Dates in any common format (DD-Mon-YYYY, MM/DD/YYYY, YYYY-MM-DD, etc.) are automatically parsed. If the spreadsheet has no "Title" column but has a description column (e.g., "Risk Description"), the description is used as the title.
...
In addition to the standard action types, you can add an **AI Generate** action to have AI produce content as part of your automation — summaries, recommendations, draft messages, or any other text.

**How to set it up:**

1. Add a new action and choose **AI Generate** from the action type list.
2. Write a **prompt** describing what you want the AI to generate. You can reference trigger data using template variables (e.g., `{{task.name}}`, `{{task.status}}`).
3. Set an **output key** — a short identifier for the result (defaults to `output`). This is the key you use to reference the generated text in later actions.
4. In any subsequent action (e.g., Send Email or Add Comment), reference the AI output using `{{ai.output}}` — or `{{ai.your_key}}` if you chose a custom output key.

**Example:** An automation that fires when a risk is created could use AI Generate with the prompt "Summarise this risk and suggest one mitigation: {{risk.description}}" and then pass `{{ai.output}}` as the body of a notification.
...
Each automation has an **Insights** tab that surfaces performance data at a glance, so you can understand how reliably your rules are firing and where problems occur.

**What you'll see:**

- **Total runs** — the cumulative number of times the rule has executed.
- **Success rate** — the percentage of runs that completed without error.
- **Average duration** — the typical time (in milliseconds) from trigger to completion.
- **Common errors** — a ranked list of the most frequently occurring error messages, to help you quickly diagnose misconfigured actions or recipient addresses.
- **Daily activity chart** — a bar chart showing run counts for each of the last 30 days, so you can spot unusual spikes or gaps in activity.

**Dry-run condition trace:** When you click **Test** to run a dry-run, the result now includes a **condition trace tree**. Each condition in your rule is shown as a node with a pass or fail indicator and the actual value that was evaluated — so you can see exactly why a condition passed or was skipped, without having to guess.

### AI Builder

If you prefer not to configure a trigger, conditions, and actions manually, use the **AI Builder** to describe your automation in plain English and let the AI generate the configuration for you.

**How to use it:**

1. Click **New Automation**, then toggle on **AI Builder** at the top of the form.
2. Type a plain-English description of what you want the automation to do. For example:
   - *"When a task becomes overdue, send an email to the project owner"*
   - *"If a risk is created with severity High, add a comment to the risk and notify the project manager"*
3. Click **Generate**. The AI produces a suggested trigger, conditions, and action list, and pre-fills the standard automation form with the result.
4. Review the generated configuration — adjust anything that doesn't look right — and click **Save** when you're happy.
...
On the **Automations** list page, a **Suggested Automations** section appears below your existing rules. Suggestions are generated by analysing your project's current state — open risks, task completion patterns, team size, and recent activity — and recommending automations that would be useful for a project like yours.

**Reading a suggestion:**

- Each card shows the automation name, a brief description of what it would do, and a **confidence level** (Low / Medium / High) reflecting how well it matches your project's situation.
- A short **reason** explains why the suggestion was generated (e.g., "You have 4 overdue tasks with no escalation rule").

**Acting on a suggestion:**

- **Apply** — creates a draft automation pre-filled with the suggested configuration. The automation is saved in a disabled state so you can review and edit it before enabling.
- **Dismiss** — hides the suggestion for 30 days. It may reappear later if the underlying conditions persist.
