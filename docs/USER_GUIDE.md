...
- **Multi-select bulk edit**: **Ctrl+click** (Cmd+click on Mac) any task row to add or remove it from the selection — no need to use checkboxes first. **Shift+click** for range selection. You can also click the checkbox in the # header to select all rows, or click individual checkboxes. When tasks are selected, a sticky toolbar appears with dropdowns to bulk-update Status, Priority, or Assignee, plus a Delete button. Press the **Delete** key to bulk-delete selected tasks (with confirmation). Click **Clear** to deselect all.
